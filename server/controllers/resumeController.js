...
const parseResumeWithAI = async (extractedText) => {
  const prompt = `Extract structured information from the following resume text and return a JSON object with this exact structure (no extra text, no markdown, no code blocks):
{
  "contactInfo": {
    "name": "string or null",
    "email": "string or null",
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert resume parser. Always respond with valid JSON only. No markdown, no code blocks, no extra text.",
        },
        {
          role: "user",
          content: prompt,
