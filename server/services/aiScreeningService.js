...
const runAIScreening = async (applicationId) => {
  ...
  try {
    ...
    try {
      ...
      throw new Error(`Failed to parse Groq response: ${rawText}`);
    }

    const { matchScore, matchSummary, keywordsFound, keywordsMissing, strengths, improvements } =
      parsed;

    if (typeof matchScore !== "number" || matchScore < 0 || matchScore > 100) {
      throw new Error("Invalid matchScore in Groq response");
    }
    ...
    console.log(
      `✅ AI screening completed for application ${applicationId} — Score: ${matchScore}`
    );
